...
_LINDA_RASCHKE_LESSONS: tuple[_LessonSpec, ...] = (
    _LessonSpec(
        ...
        deeper_explanation="app/gatekeeper.py's evaluate_gatekeeper() is TradeTown's own real, disclosed pre-trade checklist: thirteen independent checks (decision confidence, risk-manager alignment, multi-agent agreement, the AI Debate's outcome, portfolio exposure, correlated positions, active risk warnings, market conditions, the weighted executive recommendation, the Behavioral Circuit Breaker, the failure-boundary check, the account-level risk halt, and — CEO directive 'Layered Kill Switches' — a scoped symbol/category Trading Restriction) run on every single proposal. `approved = all(c.passed for c in checks)` — a pure AND across all thirteen; there is no override, no partial credit, and no way for one strong check to compensate for a failed one. A rejected proposal is recorded as a real, auditable GatekeeperRejection naming exactly which check(s) failed, so the reason a trade didn't happen is never a mystery.",
        quiz_question="How does TradeTown's Trade Gatekeeper combine its thirteen real checks into an approve/reject decision?",
        quiz_options=(
