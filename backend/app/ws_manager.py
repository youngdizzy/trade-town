...
def build_state_message(state: GameSaveState) -> dict[str, Any]:
    ...
    return {
        ...
        # Design Bible Chapter 74.5 — CEO-mutated singleton, the same
        # shape as riskLimits/constitution above.
        "visionBoard": state.vision_board.model_dump(by_alias=True),
        ...
        # CEO directive "Features 26-30," Feature 30 — the Failure
        # Review Board (app/failure_review.py).
        "failureClassifications": [c.model_dump(by_alias=True) for c in state.failure_classifications],
