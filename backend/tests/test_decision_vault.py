...
def _confidence_engine(
    *,
    ...
    return DecisionConfidence(
        ...
        factors=[
            ConfidenceFactor(name="Multi-Agent Agreement", score=agreement, weight=0.30, detail="test"),
            ConfidenceFactor(name="Technical Alignment", score=technical, weight=0.20, detail="test"),
            ConfidenceFactor(name="Risk Conditions", score=risk, weight=0.20, detail="test"),
            ConfidenceFactor(name="Research Confidence", score=research, weight=0.15, detail="test"),
            ConfidenceFactor(name="News, Macro & Sentiment", score=news, weight=0.10, detail="test"),
            ConfidenceFactor(name="Portfolio Exposure", score=exposure, weight=0.05, detail="test"),
        ],
    )
...
def _decision(
    *,
    ...
    return TradeDecision(
        ...
        orderId="pos-1",
        confidenceEngine=confidence_engine if confidence_engine is not None else _confidence_engine(),
...
def _trade(*, trade_id: str = "trade-1", symbol: str = "NEXA", pnl: float = 100.0, pnl_pct: float = 2.0, decision_id: str = "decision-proposal-1") -> PaperTrade:
    return PaperTrade(
        id=trade_id,
        symbol=symbol,
...
def _discipline_review(
    *,
    ...
    return DisciplineReview(
        ...
        tier=tier,  # type: ignore[arg-type]
        factors=[
            DisciplineFactor(id="position_sizing_discipline", name="Position Sizing Discipline", score=position_sizing_score, weight=0.2, detail="test"),  # type: ignore[arg-type]
            DisciplineFactor(id="patience", name="Patience", score=patience_score, weight=0.2, detail="test"),  # type: ignore[arg-type]
        ],
...
def _case_study(*, category: str = "acted_too_quickly", trade_id: str = "trade-1", symbol: str = "NEXA") -> CaseStudy:
    return CaseStudy(
        id=f"case-{trade_id}",
...
class TestComputeEvidenceScore:
    def test_renormalizes_over_only_the_three_evidence_factors(self) -> None:
        engine = _confidence_engine(technical=100.0, research=100.0, news=100.0, agreement=0.0, risk=0.0, exposure=0.0)
        assert compute_evidence_score(engine.factors) == 100.0

    def test_ignores_consensus_and_portfolio_factors_entirely(self) -> None:
        low_evidence_high_consensus = _confidence_engine(technical=10.0, research=10.0, news=10.0, agreement=100.0, risk=100.0, exposure=100.0)
...
class TestBuildVaultEntry:
    ...
    def test_evidence_and_confidence_scores_are_genuinely_different_numbers(self) -> None:
        engine = _confidence_engine(technical=100.0, research=100.0, news=100.0, agreement=20.0, risk=20.0, exposure=20.0, score=55.0)
    ...
    def test_capital_allocation_and_patience_grades_reuse_the_executive_grade_scale(self) -> None:
    ...
    def test_missing_decision_grade_falls_back_to_c_for_a_pre_feature_50_decision(self) -> None:
    ...
    def test_ceo_override_true_only_when_ceo_disagreed_with_ai(self) -> None:
        agreed = CeoDecisionRecord(id="ceo-1", proposalId="proposal-1", symbol="NEXA", category="stock", aiRecommendation="buy", ceoDecision="buy", agreedWithAi=True, decisionId="decision-proposal-1", createdAt=_now_iso())  # type: ignore[arg-type]
        disagreed = CeoDecisionRecord(id="ceo-2", proposalId="proposal-1", symbol="NEXA", category="stock", aiRecommendation="buy", ceoDecision="sell", agreedWithAi=False, decisionId="decision-proposal-1", createdAt=_now_iso())  # type: ignore[arg-type]
        kwargs = dict(
            entry_id="vault-trade-1",
            trade=_trade(),
            decision=_decision(),
...
def _vault_entry(
    *,
    ...
    return DecisionVaultEntry(
        ...
        liquidityContext=LiquidityRead(symbol=symbol, zones=[], sweepDetected=False, sweepDirection="none", liquidityScore=50.0, detail="test"),  # type: ignore[arg-type]
        evidenceScore=70.0,
...
class TestComputeTradeReportCard:
    def test_would_take_again_when_grade_clears_b_minus_and_no_mistake_filed(self) -> None:
        entry = _vault_entry(entry_id="v1", decision_grade="A", case_study_category=None)
        card = compute_trade_report_card(entry)
        assert card.would_take_again is True
        assert "Yes" in card.recommendation

    def test_not_would_take_again_when_a_real_mistake_was_filed_even_with_a_good_grade(self) -> None:
        entry = _vault_entry(entry_id="v1", decision_grade="A", case_study_category="acted_too_quickly")
        card = compute_trade_report_card(entry)
        assert card.would_take_again is False
        assert "acted_too_quickly" in card.recommendation

    def test_not_would_take_again_when_grade_falls_below_b_minus_even_with_no_mistake(self) -> None:
        entry = _vault_entry(entry_id="v1", decision_grade="D", case_study_category=None)
        card = compute_trade_report_card(entry)
        assert card.would_take_again is False
        assert "B-" in card.recommendation

    def test_a_success_case_study_category_never_counts_as_a_mistake(self) -> None:
        entry = _vault_entry(entry_id="v1", decision_grade="A", case_study_category="disciplined_process")
        card = compute_trade_report_card(entry)
        assert card.would_take_again is True

    def test_overall_trade_quality_mirrors_decision_grade_not_a_separately_invented_composite(self) -> None:
...
class TestFindSimilarVaultEntries:
    def test_tier1_exact_symbol_regime_and_tier_match_wins_when_it_has_enough_matches(self) -> None:
        vault = [_vault_entry(entry_id=f"v{i}", symbol="NEXA", market_regime="sideways_range", confidence_tier="strong") for i in range(3)]
        vault += [_vault_entry(entry_id="other", symbol="ZYX", market_regime="sideways_range", confidence_tier="strong")]
        matches, matched_on = find_similar_vault_entries(vault, symbol="NEXA", market_regime="sideways_range", confidence_tier="strong")
    ...
    def test_falls_through_to_tier2_any_symbol_when_tier1_is_too_thin(self) -> None:
        vault = [_vault_entry(entry_id=f"v{i}", symbol="NEXA", market_regime="sideways_range", confidence_tier="strong") for i in range(1)]
        vault += [_vault_entry(entry_id=f"o{i}", symbol="ZYX", market_regime="sideways_range", confidence_tier="strong") for i in range(3)]
        matches, matched_on = find_similar_vault_entries(vault, symbol="NEXA", market_regime="sideways_range", confidence_tier="strong")
        assert len(matches) == 4
        assert matched_on == ["marketRegime", "confidenceTier"]

    def test_falls_through_to_tier3_confidence_tier_alone_when_tier2_is_also_too_thin(self) -> None:
        vault = [_vault_entry(entry_id="v0", symbol="NEXA", market_regime="sideways_range", confidence_tier="strong")]
        vault += [_vault_entry(entry_id=f"o{i}", symbol="ZYX", market_regime="expansion", confidence_tier="strong") for i in range(3)]
        matches, matched_on = find_similar_vault_entries(vault, symbol="NEXA", market_regime="sideways_range", confidence_tier="strong")
        assert len(matches) == 4
        assert matched_on == ["confidenceTier"]

    def test_exclude_id_removes_the_trade_comparing_against_itself(self) -> None:
        vault = [_vault_entry(entry_id=f"v{i}", symbol="NEXA", market_regime="sideways_range", confidence_tier="strong") for i in range(MIN_SIMILAR_MATCHES)]
        matches, _ = find_similar_vault_entries(vault, symbol="NEXA", market_regime="sideways_range", confidence_tier="strong", exclude_id="v0")
        assert all(m.id != "v0" for m in matches)

    def test_no_matches_anywhere_returns_empty(self) -> None:
        matches, matched_on = find_similar_vault_entries([], symbol="NEXA", market_regime="sideways_range", confidence_tier="strong")
        assert matches == []
        assert matched_on == ["symbol", "marketRegime", "confidenceTier"]
...
class TestSummarizeSimilarity:
    ...
    def test_win_rate_and_avg_pnl_are_computed_from_the_real_matches(self) -> None:
        matches = [_vault_entry(entry_id="v1", pnl=10.0, pnl_pct=2.0), _vault_entry(entry_id="v2", pnl=-5.0, pnl_pct=-1.0)]
    ...
    def test_warning_fires_only_once_a_mistake_category_clears_the_share_threshold(self) -> None:
        below_threshold = [_vault_entry(entry_id="v1", case_study_category="acted_too_quickly")] + [_vault_entry(entry_id=f"v{i}") for i in range(2, 6)]
        ...
        above_threshold = [_vault_entry(entry_id=f"m{i}", case_study_category="acted_too_quickly") for i in range(mistake_count)]
        above_threshold += [_vault_entry(entry_id=f"c{i}") for i in range(n - mistake_count)]
    ...
    def test_success_case_study_categories_never_count_toward_the_mistake_warning(self) -> None:
        matches = [_vault_entry(entry_id=f"v{i}", case_study_category="disciplined_process") for i in range(5)]
        summary = summarize_similarity(matches, ["symbol"])
