...
from app.backtest_primitives import aggregate_bucket, chandelier_stop, simulate_exit
...
class TestChandelierStop:
    def test_long_stop_is_highest_high_minus_atr_multiple(self) -> None:
        ...
        stop = chandelier_stop(candles, atr_vals, entry_index=64, direction="long", atr_period=CHANDELIER_ATR_PERIOD, atr_multiplier=CHANDELIER_ATR_MULTIPLIER)
    ...
    def test_short_stop_is_lowest_low_plus_atr_multiple(self) -> None:
        candles = _short_setup_series()
        atr_vals = atr_series(candles, CHANDELIER_ATR_PERIOD)
        stop = chandelier_stop(candles, atr_vals, entry_index=64, direction="short", atr_period=CHANDELIER_ATR_PERIOD, atr_multiplier=CHANDELIER_ATR_MULTIPLIER)
        assert stop is not None
    ...
    def test_none_with_insufficient_atr_history(self) -> None:
        candles = _long_setup_series()
        atr_vals = atr_series(candles, CHANDELIER_ATR_PERIOD)
        assert chandelier_stop(candles, atr_vals, entry_index=5, direction="long", atr_period=CHANDELIER_ATR_PERIOD, atr_multiplier=CHANDELIER_ATR_MULTIPLIER) is None
...
class TestSimulateExit:
    ...
    def test_short_direction_mirrors_long(self) -> None:
        ...
        result = simulate_exit("short", entry_price=100.0, stop_price=105.0, target_price=90.0, path=path)
        assert result.outcome == "win"
...
class TestAggregateBucket:
    def test_no_closed_trades_reads_honestly_empty(self) -> None:
        bucket = aggregate_bucket("empty", [_trade("open", 0.0)])
    ...
    def test_real_win_rate_expectancy_and_profit_factor(self) -> None:
        ...
        bucket = aggregate_bucket("mixed", trades)
    ...
    def test_below_the_bucket_verdict_bar_reads_not_enough_evidence(self) -> None:
        ...
        bucket = aggregate_bucket("small", trades)
