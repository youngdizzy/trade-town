...
def _proposal(*, symbol: str = "NEXA", score: float = 80.0, tier: str = "strong", factors: list[ConfidenceFactor] | None = None) -> TradeProposal:
    return TradeProposal(
        ...
        confidenceEngine=DecisionConfidence(score=score, tier=tier, summary="A well-supported setup.", factors=factors or _DEFAULT_FACTORS),  # type: ignore[arg-type]
        createdAt=_now_iso(),
        createdSimMinutes=0,
    )
...
def _candles(closes: list[float], symbol: str = "NEXA") -> list[Candle]:
    ...
    for i, close in enumerate(closes):
        candles.append(Candle(symbol=symbol, timeframe="1h", timestamp=f"2026-01-01T{i:02d}:00:00Z", open=close, high=close + 1, low=close - 1, close=close, volume=1000.0, data_status="simulated"))
    return candles


def _simulation(symbol: str = "NEXA") -> WhatIfSimulation:
    return run_whatif_simulation(symbol, _candles([100.0 + i * 0.5 for i in range(30)], symbol=symbol))


def _scenario(*, low: float = -5.0, high: float = 10.0, drawdown: float = -3.0, probability: float = 60.0) -> ScenarioResult:
...
class TestBuildExpectedValueAnalysis:
    ...
    def test_risk_to_reward_is_zero_when_average_drawdown_is_zero(self) -> None:
        simulation = WhatIfSimulation(symbol="NEXA", holdBars=24, scenarios=[_scenario(drawdown=0.0)], baseline=_scenario(), bestCaseScenario="bullish_continuation", worstCaseScenario="bullish_continuation")  # type: ignore[arg-type]
        analysis = build_expected_value_analysis(simulation)
    ...
    def test_edge_is_the_gap_between_expected_value_and_the_baseline(self) -> None:
        ...
        simulation = WhatIfSimulation(symbol="NEXA", holdBars=24, scenarios=[scenario], baseline=baseline, bestCaseScenario="bullish_continuation", worstCaseScenario="bullish_continuation")  # type: ignore[arg-type]
        analysis = build_expected_value_analysis(simulation)
    ...
    def test_detail_mentions_edge_or_shortfall_honestly(self) -> None:
        simulation = _simulation()
        analysis = build_expected_value_analysis(simulation)
        assert "edge" in analysis.detail.lower() or "shortfall" in analysis.detail.lower()
...
class TestBuildDecisionScore:
    ...
    def test_critical_risk_warning_for_this_symbol_tanks_risk_score(self) -> None:
        ...
        warnings = [RiskWarning(id="w1", symbol="NEXA", severity="critical", message="Breach.", createdAt=_now_iso())]  # type: ignore[arg-type]
        score = build_decision_score(
            proposal,
    ...
    def test_risk_warning_for_a_different_symbol_is_ignored(self) -> None:
        proposal = _proposal(symbol="NEXA")
        warnings = [RiskWarning(id="w1", symbol="OTHER", severity="critical", message="Breach.", createdAt=_now_iso())]  # type: ignore[arg-type]
        score = build_decision_score(
            proposal,
            risk_warnings=warnings,
            correlated_open_positions=0,
    ...
    def test_portfolio_compatibility_score_penalizes_per_correlated_position(self) -> None:
        proposal = _proposal()
        expected_value = build_expected_value_analysis(_simulation())
        no_overlap = build_decision_score(proposal, risk_warnings=[], correlated_open_positions=0, expected_value=expected_value, market_intelligence=default_market_intelligence_state(), liquidity=None)
        some_overlap = build_decision_score(proposal, risk_warnings=[], correlated_open_positions=2, expected_value=expected_value, market_intelligence=default_market_intelligence_state(), liquidity=None)
    ...
    def test_strategy_health_score_is_always_none_for_ordinary_proposals(self) -> None:
        proposal = _proposal()
        score = build_decision_score(proposal, risk_warnings=[], correlated_open_positions=0, expected_value=build_expected_value_analysis(_simulation()), market_intelligence=default_market_intelligence_state(), liquidity=None)
    ...
    def test_liquidity_quality_reads_the_real_liquidity_score_when_present(self) -> None:
        proposal = _proposal()
        liquidity = LiquidityRead(symbol="NEXA", zones=[], sweepDetected=False, sweepDirection="none", liquidityScore=90.0, detail="test")  # type: ignore[arg-type]
        score = build_decision_score(proposal, risk_warnings=[], correlated_open_positions=0, expected_value=build_expected_value_analysis(_simulation()), market_intelligence=default_market_intelligence_state(), liquidity=liquidity)
...
class TestBuildContingencyPlan:
    ...
    def test_liquidity_sweep_step_triggers_off_the_real_liquidity_read(self) -> None:
        liquidity = LiquidityRead(symbol="NEXA", zones=[], sweepDetected=True, sweepDirection="above_highs", liquidityScore=50.0, detail="test")  # type: ignore[arg-type]
    ...
    def test_high_volatility_regime_triggers_the_volatility_step(self) -> None:
        market_intelligence = default_market_intelligence_state().model_copy(update={"regime": "high_volatility"})
    ...
    def test_liquidity_hunt_regime_triggers_the_liquidity_hunt_step(self) -> None:
        market_intelligence = default_market_intelligence_state().model_copy(update={"regime": "liquidity_hunt"})
        steps = build_contingency_plan(market_intelligence, None)
    ...
    def test_elevated_news_risk_triggers_the_news_step(self) -> None:
        news_risk = NewsRiskRead(activeMarketNewsCount=3, riskLevel="elevated", detail="test")  # type: ignore[arg-type]
        market_intelligence = default_market_intelligence_state().model_copy(update={"news_risk": news_risk})
        steps = build_contingency_plan(market_intelligence, None)
    ...
    def test_avoid_trading_quality_tier_triggers_the_quality_step(self) -> None:
        market_intelligence = default_market_intelligence_state()
        quality = market_intelligence.quality.model_copy(update={"tier": "avoid_trading"})
        market_intelligence = market_intelligence.model_copy(update={"quality": quality})
        steps = build_contingency_plan(market_intelligence, None)
...
class TestEvidenceNeverExceedsConfidence:
    ...
    def test_holds_even_when_evidence_factors_score_lower_than_consensus_factors(self) -> None:
        factors = [
            ConfidenceFactor(name="Multi-Agent Agreement", score=100.0, weight=0.30, detail="d"),
            ConfidenceFactor(name="Technical Alignment", score=10.0, weight=0.20, detail="d"),
            ConfidenceFactor(name="Risk Conditions", score=100.0, weight=0.20, detail="d"),
            ConfidenceFactor(name="Research Confidence", score=10.0, weight=0.15, detail="d"),
            ConfidenceFactor(name="News, Macro & Sentiment", score=10.0, weight=0.10, detail="d"),
            ConfidenceFactor(name="Portfolio Exposure", score=100.0, weight=0.05, detail="d"),
        ]
...
class TestCompareScenarioToOutcome:
    def test_within_predicted_range_true_when_outcome_falls_inside_the_closest_scenario(self) -> None:
        simulation = _simulation()
        closest = min([*simulation.scenarios, simulation.baseline], key=lambda s: abs((s.reward_range_low_pct + s.reward_range_high_pct) / 2))
        midpoint_pnl = (closest.reward_range_low_pct + closest.reward_range_high_pct) / 2
    ...
    def test_within_predicted_range_false_when_outcome_lands_outside_every_range(self) -> None:
        simulation = _simulation()
