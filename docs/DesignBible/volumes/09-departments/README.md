...
| 61 | [Institutional Knowledge Graph & Company Memory Engine](chapter-61-knowledge-graph-company-memory.md) | Partially implemented — Knowledge Graph extension (backend `app/knowledge_graph.py` + frontend), Pattern Detection Sensitivity CEO controls, and the Decision Vault slice of Knowledge Retention Rules shipped; the Company Memory slice of Knowledge Retention Rules and Quality Score still target design |
