...
Each agent wanders gently within its current room and shows a name tag —
but only when the player is within `NAME_TAG_VISIBLE_RADIUS` (32px). Rooms
like Brain Room and Meeting Room can legitimately hold all four agents at
once (that's the intended "Mission Control"/meeting design), and their
sprites sit closer together than a name tag is wide; rather than fight
that with ever-increasing spacing, tags only show up close, the same
convention used by Stardew-style top-down games, so a crowded room reads
as a crowd instead of a wall of overlapping text. Interacting opens the
...
- **News** (`NewsItem`): `category` is `"company"` | `"discovery"` |
  ...
  completes a task; `market` items are static placeholder copy (no live
  feed yet, by design — see NEXUS above); `company` is reserved for future
  company-level events. The Lobby newspaper stand and Brain Room HUD both
  read the same feed, grouped by category.
