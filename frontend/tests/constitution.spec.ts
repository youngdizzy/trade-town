...
test("the Constitution's 14 real Articles (I-VIII plus the Probability First Trading Philosophy's IX-XIII plus the CEO directive's XIV) are present in the backend state", async ({ page }) => {
  await page.goto("/");
  ...
  expect(state.constitution.articles.map((a: { id: string }) => a.id)).toEqual(["I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII", "XIII", "XIV"]);
